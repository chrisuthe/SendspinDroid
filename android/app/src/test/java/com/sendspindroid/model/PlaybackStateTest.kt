...
class PlaybackStateTest {
    ...
    fun withClearedMetadata_clearsAllMetadata() {
        ...
        // Group info should be preserved
    ...
    fun interpolatedPositionMs_whenPlaying_addsElapsedTime() {
        // SystemClock.elapsedRealtime() returns 10_000 (from setUp)
        val state = PlaybackState(
            playbackState = PlaybackStateType.PLAYING,
            positionMs = 5000,
            positionUpdatedAt = 8_000L, // 2 seconds ago
            durationMs = 180000
        )
        assertEquals(7000L, state.interpolatedPositionMs) // 5000 + 2000
    }
    ...
    fun interpolatedPositionMs_whenPlaying_cappedAtDuration() {
        val state = PlaybackState(
            playbackState = PlaybackStateType.PLAYING,
            positionMs = 179999,
            positionUpdatedAt = 5_000L, // 5 seconds ago
            durationMs = 180000
        )
        assertEquals(180000L, state.interpolatedPositionMs) // capped
    }
}
